import { isEmail, isComplexPassword } from './util/validate';
...
class UserAPI {
  ...
  create(user, cb) {
    if (!(user.name && user.email && user.password) ||
        !(isEmail(user.email) && isComplexPassword(user.password))) {
  ...
  update(id, user, cb) {
    ...
    this.db.updateUser(parseInt(id), user, (err, results) => {
      if (err && err.code === 'ER_DUP_ENTRY') {
        return cb(409, {err: 'User already exists'});
      } else if (err) {
        return cb(500, {err: 'Server error'});
  ...
  remove(id, cb) {
    ...
    this.db.deleteUser(parseInt(id), (err, results) => {
      if (err) {
        return cb(500, {err: 'Server error'});
      }
