...
class UserAPI {
  ...
  sendResetCode(id, cb) {
    ...
    this.db.generateResetCode(id, (err, code) => {
      if (err) {
        return cb(500, {err: 'Server error'});
      }
  ...
  resetPassword(id, body, cb) {
    ...
    if (body.password && !isComplexPassword(body.password)) {
      errors.password = 'Password must be at least 6 characters long,\n' +
                        'have at least one uppercase letter,\n' +
                        'have at least one lowercase letter,\n' +
                        'have at least one number.';
    ...
    if (!(body.password && body.password2) ||
        Object.keys(errors).length)
    {
      return cb(400, {err: 'Invalid password reset', errors});
    }

    this.db.resetPassword(id, body.password, (err, updated) => {
      if (err) {
        return cb(500, {err: 'Server error'});
      }
